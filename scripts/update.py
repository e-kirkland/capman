...
def update_from_transctions(transactions, lastTransaction):
    ...
    new_transactions = [x for x in transactions if int(x['transaction_id']) > int(lastTransaction)]
...
def check_compilance():
    ...
    for n in range(0, len(roster_ids)):
        ...
        if (salary_total>cap) | (players_total<rosterMin) | (players_total>rosterMax):
...
def update_settings(salaryCap, rosterMin, rosterMax):
    ...
    settingsdf['salary_cap'] = settingsdf['salary_cap'].replace([oldcap], int(salaryCap))
    settingsdf['roster_min'] = settingsdf['roster_min'].replace([oldmin], int(rosterMin))
    settingsdf['roster_max'] = settingsdf['roster_max'].replace([oldmax], int(rosterMax))
